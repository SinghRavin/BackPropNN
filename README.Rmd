...
Where, [] represents the matrix, x represents the Hadamard multiplication (element-wise), and . represents the usual matrix multiplication.
...
We see that a better AUC score is achieved by the BackPropNN package than R nnet, however a better mse is achieved by R nnet (achieving slightly lower value than BackPropNN).
...
We notice that for the training part, Rcpp version achieves best computational speed, however for the predicting part, R nnet package takes the lead (performing just slightly better than Rcpp). The Original package written in R lags behind in both training and predicting tasks.
