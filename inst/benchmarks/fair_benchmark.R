...
##
## Every measurement below uses the same 70/30 split, the same seeds, and the
## same high-resolution clock, so a given row means the same thing in every
## table.
##
...
## ---- Train / test split ---------------------------------------------------
## Used by every measurement below: timing, accuracy, and cost.
set.seed(100)
...
res_train <- bench::mark(
  "Pure R" = { set.seed(1); back_propagation_training(i, h, o, learning_rate,
                                                      activation_func, train, epochs = E) },
  "Rcpp"   = { set.seed(1); back_propagation_training_rcpp(i, h, o, learning_rate,
                                                           activation_func, as.matrix(train),
                                                           epochs = E) },
  "nnet"   = { set.seed(1); nnet::nnet(X_tr, Y_tr, size = h, maxit = E, trace = FALSE) },
...
set.seed(1); m_cpp  <- back_propagation_training_rcpp(i, h, o, learning_rate,
                                                      activation_func,
                                                      as.matrix(train), epochs = E)
set.seed(1); m_nnet <- nnet::nnet(X_tr, Y_tr, size = h, maxit = E, trace = FALSE)

res_pred <- bench::mark(
  "Pure R" = feed_forward(test, m_r),
  "Rcpp"   = feed_forward_rcpp(test, m_cpp),
  "nnet"   = as.numeric(stats::predict(m_nnet, X_te, type = "raw")),
...
## ---- Accuracy -------------------------------------------------------------
## Evaluated on the held-out 30%, averaged over five random initialisations.
## The C++ engine is used for the repeated fits because it produces weights
## identical to the R engine at a fraction of the cost.
score <- function(p) {
...
## Timing uses bench::mark's high-resolution clock rather than system.time(),
## whose ~15 ms granularity on Windows is too coarse for the fastest fits.
time_fit <- function(f, reps = 5) {
  b <- bench::mark(f(), iterations = reps, check = FALSE,
...
results <- list(
  ...
  settings      = list(num_obs = num_obs, n_train = nrow(train), n_test = nrow(test),
                       i = i, h = h, o = o,
